...
for i = 1:n-1
    ...
    elseif y2(i) >= y3(i)&&y1(i) >= y3(i)&&y3(i) <= 0
        Ft = -N*k*(sqrt(R*R+y3(i)*y3(i))-R)*y3(i)/sqrt(R*R+y3(i)*y3(i));     % 弹簧力
        Fa = Pi*Ca*R*R*R*v3(i)*v3(i)*sign(v3(i))/(sqrt(R*R+y3(i)*y3(i))*6);
        Fd = -Cd*v3(i)*sign(v3(i));
        Fc = 2.5*sign(v3(i));
        Fm2 = - Ca*Sh*(v2(i)*v2(i))*sign(v2(i));
        Fm1 = - Ca*Sh*(v1(i)*v1(i))*sign(v1(i));
        a2 = -g + Fm2/m2;                      % 物体1的加速度
    ...
    if v3(i)<=0&&v3(i+1)>=0&&y2(i) >= y3(i)&&y1(i) >= y3(i)
    y3(i + 1) = y3(i) + (v3(i)+v3(i+1))/2 * dt-y3(i)-((E3+28800*y3(i)^4)^0.25)/(2*sqrt(2*sqrt(2))*sqrt(15)); % 更新物体1的位置
    else
    y3(i + 1) = y3(i) + (v3(i)+v3(i+1))/2 * dt; % 更新物体1的位置
    end
end
% 绘制结果
figure;
...
xlabel('Time (s)');
ylabel('Position (m)');
title('Three kids on the trampoline');
